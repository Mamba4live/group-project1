...
author: "Yinbo Zhao 520173668" "Tierui Yang 520197398"
subtitle: "Project 1"
...
```{r}
abbp=data.frame(bp)
abbp <- abbp[which(bp>=90),]
length(abbp)/length(bp)
```
...
the rate of people with abnormal Insulin
```{r}
abins=data.frame(ins)
abins <- abins[which(ins>=150),]
length(abins)/length(ins)
```
