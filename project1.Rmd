...
par(mfrow = c(1,2))
hist(BloodPressure)
boxplot(BloodPressure,horizontal = T)
```
...
```{r}
curve(dnorm(x,round(mean(BloodPressure),0),round(sd(BloodPressure),0)),from=0,to=140,ylab="Density",main="Normal Curve of BloodPressure")
```
...
Lastly, the rate of people with abnormal Blood Pressure (Hypertension- elevated Bloodpressure)shows:
```{r}
...
boxplot(BloodPressure ~ Outcome, horizontal=T, col=c("green", "red"), main = "Classification (Outcome attribute) by BloodPressure")
```
...
- and the rate of people with abnormal Glucose:
```{r}
...
```{r}
Outcome = factor(f$Outcome, levels = c("0", "1"))
boxplot(Glucose ~ Outcome, horizontal=T, col=c("green", "red"), main = "Classification (Outcome attribute by Glucose ")
```
<br>
...
**1. Scatter Plot:** 
```{r}
plot(Glucose, BloodPressure, xlab = "Glucose (mg/dL)",ylab = "BP (mm Hg)")
```
...
**2. HeatMap:**
```{r}
heatmap(table(Glucose, BloodPressure), Rowv = NA, Colv = NA)
```
...
```{r}
Outcome = factor(f$Outcome, levels = c("0", "1"))
boxplot(Insulin ~ Outcome, horizontal = T, col= c("green", "red"), main = "Classification (Outcome attribute) by Insulin")
```
...
**2. HeatMap:**
```{r}
heatmap(table(Insulin,Glucose), Rowv = NA, Colv = NA)
```
