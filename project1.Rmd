...
68% people's BloodPressure is in the range [58, 83]
```{r}
mean(bp)-sd(bp)
mean(bp)+sd(bp)

```
95% people's BloodPressure is in the range [46, 96]
```{r}
mean(bp)-2*sd(bp)
mean(bp)+2*sd(bp)
```
99.7% people's BloodPressure is in the range [33, 108]
```{r}
mean(bp)-3*sd(bp)
mean(bp)+3*sd(bp)
```
...
```{r}
curve(dnorm(x,round(mean(ins),0),round(sd(ins),0)),from=0,to=312,ylab="Density",main="Normal Curve of Insulin")
